...
def logmgf_exact(q, priv_eps, l):  # noqa: E741
...
def logmgf_from_counts(counts, noise_eps, l):  # noqa: E741
    """
...
def sens_at_k(counts, noise_eps, l, k):  # noqa: E741
...
def smoothed_sens(counts, noise_eps, l, beta):  # noqa: E741
...
def perform_analysis(
    teacher_preds, indices, noise_eps, delta=1e-5, moments=8, beta=0.09
):
    """ "Performs PATE analysis on predictions from teachers and combined predictions for student.
    Args:
        teacher_preds: a numpy array of dim (num_teachers x num_examples).
            Each value corresponds to the index of the label which a teacher
            gave for a specific example
        indices: a numpy array of dim (num_examples) of aggregated examples
            which were aggregated using
    ...
    for i in indices:
        ...
        total_log_mgf_nm += np.array(
            [
                logmgf_from_counts(counts_mat[i], noise_eps, l)  # noqa: E741
        ...
        total_ss_nm += np.array(
            [
                smoothed_sens(counts_mat[i], noise_eps, l, beta)  # noqa: E741
                for l in l_list  # noqa: E741
            ]
        )
    ...
    data_ind_log_mgf += num_examples * np.array(
        [logmgf_exact(1.0, 2.0 * noise_eps, l) for l in l_list]  # noqa: E741
    )
