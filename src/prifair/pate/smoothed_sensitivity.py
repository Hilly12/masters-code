...
def _compute_data_dep_bound_gnmax(sigma, logq, order):
    ...
    log_a = (order - 1) * (log1q - (np.log1p(-math.exp((logq + eps2) * (1 - 1 / mu2)))))
...
def compute_logq0_gnmax(sigma, order):
    ...
    def _check_validity_conditions(logq):
        ...
        # from Appendix.
        return logq <= (mu2 - 1) * eps2 - mu2 * math.log(
            mu1 / (mu1 - 1) * mu2 / (mu2 - 1)
        )

    def _compare_dep_vs_ind(logq):
        return _compute_data_dep_bound_gnmax(
            sigma, logq, order
        ) - pate.rdp_data_independent_gaussian(sigma, order)

    # Natural upper bounds on q0.
    logub = min(
        -((1 + 1.0 / sigma) ** 2), -(((order - 0.99) / sigma) ** 2), -1 / sigma**2
    )
...
def _compute_local_sens_gnmax(logq, sigma, num_classes, order):
    ...
    beta_bu_q = _compute_rdp_gnmax(
        sigma, math.log(_compute_bu_gnmax(math.exp(logq), sigma, num_classes)), order
    )
    beta_bl_q = _compute_rdp_gnmax(
        sigma, math.log(_compute_bl_gnmax(math.exp(logq), sigma, num_classes)), order
    )
...
def compute_local_sensitivity_bounds_threshold(
    counts, num_teachers, threshold, sigma, order
):
...
dict_beta_discount: Dict[Any, Any] = {}
...
def compute_rdp_of_smooth_sensitivity_gaussian(beta, sigma, order):
    ...
    return order * math.exp(2 * beta) / sigma**2 + (
        -0.5 * math.log(1 - 2 * order * beta) + beta * order
    ) / (order - 1)
...
def _is_non_decreasing(fn, q, bounds):
    ...
    diff_fn_lambdified = sp.lambdify(
        ...
        modules=[
            "numpy",
            {"erfc": scipy.special.erfc, "erfcinv": scipy.special.erfcinv},
        ],
    )
